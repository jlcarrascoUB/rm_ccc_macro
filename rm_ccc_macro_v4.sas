...
repeated measurements. The CCC is computed using both (1) U-statistic methodology
(King et al., 2007) and (2) an appropriate specification of the intraclass
correlation coefficient from a variance components linear mixed model
(Carrasco et al., 2009).

Both approaches are produced except in specific situations. For missing or
unbalanced data, or for multiple observers/raters, the U-statistic method is not
appropriate. For small samples (n<20), both CCCs are produced, but the
U-statistic method is recommended, and this is noted in the log. The U-statistic
method may also be preferable for non-normal or qualitative data, although this
is left to the user's discretion.

Both methods are appropriate for an identity or diagonal weight matrix, D.
Because the variance components approach is not currently developed for a general
weight matrix, only the U-statistic approach is valid when D is of a general form.

If repeated measurements are replicated but are not longitudinal in nature, this
should be specified using the NL option. In this case, the variance components
CCC is estimated using the %ccc macro.

For nonlongitudinal replicated data (NL=1), the INTERACTION parameter determines
whether the subject-by-observer interaction is included in the mixed model when
more than one replicated measurement per subject and observer is available:

    interaction=1 (default): include the subject-by-observer interaction.
    interaction=0:           exclude the subject-by-observer interaction.

The INTERACTION parameter has no effect for longitudinal analyses (NL=0).
...
the measurement variable, observer/method variable, a discrete time variable,
and subject identification variable.
...
TIME=           Discrete time variable or, in the case of nonlongitudinal data,
                a variable differentiating the repeated measurements.

SUBJECT=        Subject identifier.

WEIGHT=         A data set with p rows and p columns representing a p x p general
                weight matrix, where p is the number of repeated measures per
                method and subject. If no weight matrix is specified, and no
                diagonal weight matrix is specified, an identity matrix is used.
                (See also DIAG.)

DIAG=           An alternative way to enter a diagonal weight matrix. A list of p
                weights should be listed within parentheses, separated by commas,
                where p is the number of repeated measures per method and subject.
                If no weight matrix is specified through either the WEIGHT or
                DIAG options, an identity matrix is used.
...
NL=             0 if data are longitudinal, 1 if data are nonlongitudinal.
                The default is 0.

TYPE=           TYPE=0 for no repeated statement, TYPE=1 for autoregressive
                (AR(1)), or TYPE=2 for compound symmetry covariance structure.
                This option has no effect if NL=1.

INTERACTION=    Used only for nonlongitudinal replicated data (NL=1).
                INTERACTION=1 includes the subject-by-observer interaction in
                the variance components model when m>1.
                INTERACTION=0 fits the reduced model without the
                subject-by-observer interaction.
                The default is 1.

OUTPUT=         Specifies an output dataset. If no dataset is specified, no
                output dataset is created.

MODEL_DETAIL=   OFF for simple output. If ON is specified, model details will be
                printed for the linear mixed model used in the variance
                components method.
...
%put interaction is: &interaction;


/* Check interaction parameter */
%if &interaction ne 0 and &interaction ne 1 %then %do;
    %put;
    %put ERROR: INTERACTION MUST BE 0 OR 1.;
    %put;
    %return;
%end;
...
proc iml;
    ...
    **************************************************************************/
    %if &weight ne %then %do;
        ...
        if d=diag(d) then
            call symputx('indic',1);
        else
            call symputx('indic',0);
    ...
    %if &ncol ^= &maxp %then %do;
        ...
        %put;
        %put ERROR: THE WEIGHT MATRIX IS OF THE WRONG DIMENSION. THE MACRO WILL TERMINATE.;
        %put;
...
/* Check whether at least one methodology is valid */
...
/*******************************************************************************
  Delete intermediate output datasets if they are not requested as final output
*******************************************************************************/
%if &output ne _vcout and &output ne _uout %then %do;
...
/***********************************************************************************************
CCC estimates the Concordance Correlation Coefficient (CCC) through variance components 
obtained from a linear mixed model with subject and observer effects. Observers are considered 
as fixed effects.

If m=1, where m is the number of repeated measurements per subject and observer, the 
subject-by-observer interaction cannot be estimated separately from the residual error and 
the model includes only the subject random effect.

If m>1, the subject-by-observer interaction can be included or excluded using the 
INTERACTION parameter:

   interaction=1 (default): the subject-by-observer interaction is included in the model.
   interaction=0:           the subject-by-observer interaction is excluded from the model.

Thus, when interaction=0, the CCC is estimated from the variance components of the reduced 
model without the subject-by-observer interaction, regardless of the value of m.
************************************************************************************************/;%macro ccc(infile, output=ccc, m=1, interaction=1, detail=off);

%put NOTE: ccc detail is: &detail.;
%put NOTE: interaction is: &interaction.;

/* Check interaction parameter */
%if &interaction ne 0 and &interaction ne 1 %then %do;
    %put ERROR: The interaction parameter must be 0 or 1.;
    %return;
%end;

%if &detail=off or &detail=OFF or &detail=Off %then %do;
    ods select none;
%end;
...
proc mixed data=&infile asycov method=reml;
    ...
    /*
       Without subject-by-observer interaction:
       - necessarily when m=1
       - optionally when interaction=0
    */
    %if &m=1 or &interaction=0 %then %do;
        random ind;
    %end;

    /*
       With subject-by-observer interaction
    */
    %else %if &m>1 and &interaction=1 %then %do;
        random ind ind*met;
    %end;
...
proc iml;
    ...
    use _table1;
    read all var{'Estimate'} into betaaux;
    ...
    /*
       No subject-by-observer interaction
    */
    %if &m=1 or &interaction=0 %then %do;
    ...
    /*
       Subject-by-observer interaction included
    */
    %else %if &m>1 and &interaction=1 %then %do;
    ...
    var_sb=
        ((2*trace((A*VarB)**2))
        +(4*(t(beta)*A*VarB*A*beta)))
        /(&m*(k*(k-1))**2);


    /*------------------------------------------------------------------------------------------
      Model WITH subject-by-observer interaction
    ------------------------------------------------------------------------------------------*/

    %if &m>1 and &interaction=1 %then %do;
    ...
    /*------------------------------------------------------------------------------------------
      Model WITHOUT subject-by-observer interaction
    ------------------------------------------------------------------------------------------*/

    %else %if &m=1 or &interaction=0 %then %do;
    ...
    /*
       Output WITH subject-by-observer interaction
    */
    ...
    %if &m>1 and &interaction=1 %then %do;
        ...
        print
            ...
            'Derived from Variance Components',,
            ccc[
                label=''
                colname=('CCC')
            ];
    ...
    /*
       Output WITHOUT subject-by-observer interaction
    */
    ...
    print
        S[
            label="Variance-Covariance Matrix of Variance Components"
            format=20.8
        ];
